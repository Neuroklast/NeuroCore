...
- Circuit is a headless graph (`boardStore` + DOM chips + one cable canvas). Do not reintroduce React Flow Handles for jack connect or `nodesDraggable` for chip move. If the board already has the primitive, use it.
