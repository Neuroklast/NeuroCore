...
- Theme engine: `signal` (red/cyan/black/white, default), `gold` (yellow/cyan/black), `azure` (blue/cyan/black), `digicide` (steel/teal/black; Unit uses the Digicide mask). Paint is `--nk-*` / `themeOf(id)` — no private hex in UI code. Settings → THEME. Frame rate is 30 or 60. Shared UI prefs (theme, motion, LIVE, fps, cables, unsaved prompt) live in `%AppData%/NEUROKLAST/NeuroKore/ui.settings`, not WebView2 localStorage. They hit every running processor, including inserts with no editor. Mix, OS, knobs, and the formula stay per insert.
