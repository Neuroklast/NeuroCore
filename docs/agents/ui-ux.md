...
| After drop | Layout worker runs ELK + A*. Canvas paints cached `edge.route` polylines; rAF only animates dashoffset. Pause the canvas when Circuit is hidden |
...
- Theme engine: `signal` (red/cyan/black/white, default), `gold` (yellow/cyan/black), `azure` (blue/cyan/black), `digicide` (steel/teal/black; Unit uses the Digicide mask). Paint is `--nk-*` / `themeOf(id)` — no private hex in UI code. Settings → THEME. Frame rate is 30 or 60. Shared UI prefs (theme, motion, LIVE, fps, cables, unsaved prompt) live in `%AppData%/NEUROKLAST/NeuroKore/ui.settings`, not WebView2 localStorage. Reload that file when another process writes it. They hit every running processor, including inserts with no editor (parked WebView stays visible for host events). Mix, OS, knobs, and the formula stay per insert.
