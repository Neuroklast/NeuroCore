...
- `processOn` hands chips a view of callback `n`, never `AudioBuffer::getNumSamples()` of prepare-sized scratch. Walking the OS ceiling on a 64-sample host callback is HF scratch and insane Circuit dB. Same view as `DspEngine` `osWork`.
...
- **Board first.** Circuit is headless (`boardStore` + DOM chips + one cable canvas). Use snap, stored path, and the layout worker before inventing a new helper. Do not reintroduce React Flow Handles or `nodesDraggable`.
...
- VST3 `Contents/Resources/moduleinfo.json` must be written by a custom target that is always out of date *and* by POST_BUILD after JUCE deletes the file. POST_BUILD alone is skipped when MSBuild considers the DLL up to date. Do not copy Serum’s CID into either product.
...
- Circuit is headless (`boardStore` + DOM chips + one cable canvas). Do not wire AssembleView back to React Flow. Layout adapters use local `flowTypes`, not `@xyflow/react`. Camera pan/zoom writes the CSS matrix and canvas `camRef` in the pointer event; `setCamera` is pointerup / fit, never every move. Chip drag is the same: `chipDragRef` + CSS translate until pointerup. Hover/DoF is `data-focus` on the DOM, not `useState`. Cable geometry (chamfer/offset) is stamped; the frame loop only moves ink. Each lane integrates its own RMS px/sec (`streamAdvance`); do not share one `now` offset across tubes or reset phase on silence. Packets are an aperiodic train (`C(k) + travel`), not a repeating dash — a regular lattice aliases and looks reversed. Per-frame travel stays under half the smallest gap. Mute is a script overlay — do not replace the visual AST, do not flip `origin` to canvas, and do not hydrate the board (keepXy must keep stored routes). Native parse drops `# nk-ms` chips from JSON; a second echo of the same muted script is still overlay (`muteOverlayOnly` includes identical strings). Do not `resetMuteSolo` on host echo. Live/Studio must not `setSize` the editor — only a scale-percent change may. Shipping VST3 serves the embedded zip (`wantDiskWebAssets` off unless `NEUROKORE_WEB_DISK=1`). Do not `remove_directory` `Contents/resources` on Windows: that path is `Contents/Resources` (moduleinfo). `layoutBoard` posts to the existing layout worker; timeout **or onerror** falls back to `runLayout` — never reject Compact/Arrange. Do not stick `workerDead`. Circuit rAF does not run on Unit or a 0-size pane (`circuitPaintActive`). Bind dest jacks are `bindJackWorld`, not `getBoundingClientRect`.
- Settings Motion Full overrides Windows `prefers-reduced-motion`. Bind `data-motion` on `<html>`. CSS must not freeze Full behind the OS media query. Parked WebViews stay `isVisible` so `emitEventIfBrowserIsVisible` still delivers shared prefs.
...
- HVH is the blocked-channel fallback only. A* is the live router. Dest left+below is the wrap rail between rows, never `aroundFallback` over the far hull (packets then run away from dest). Corners are 45° chamfers of one cell, not long diagonals.
- Pins on the 16 px grid. No `dropMicroJogs`, no 2 px jack inset. Side jacks sit on cell midlines (`sidePortLocals`). A 32-grid chip plus a grid-line jack cannot share a rail with a cell-center jack — that 16 px hop is the knick after IN. Do not snap jack locals in the packer independently of the painted ports.
...
- IO tiles do not use the DSP label-column gutter. That gutter is 44 px each side and clips "OUT" to "O._". Compact IO is one or two jacks; more mix-ins grow `ioFaceSize(n)` to `sidePortMinHeight`. Do not CSS-blur or `will-change: transform` a zoomed board — Chromium rasterizes the chip layer at 1× then scales it soft.
- Side jacks are local centres (`sidePortLocals`). Global = node origin + local. Paint and A* read the same vector. Never derive a port from the DOM. Equal pitch, inside the AABB.
- `fitView` waits until the pane has a box (Circuit is `hidden` on Unit, not unmounted) and the laid-out node ids are on the board. `BOARD_MIN_ZOOM` is one constant. Do not invent a second zoom floor to hide a missed fit.
...
- In-plugin Help is the operator guide minus Install and Troubleshooting. Empty-window / WebView2 lives in Install — you cannot read Help if the window is empty. One markdown file feeds the installer *and* Help. Do not write Overview as a note about that pipeline (“this file”, “except Install”, “already running”). Apex 400 titles, JetBrains body, cyan chips. Not a `<pre>` dump.
...
- Cubase VST3 scan of a **new path** instantiates the processor, then `createView` / `attached()` / `removed()`. Chromium belongs to the processor (NEUROMETER model): birth it in the holder ctor on a hidden park HWND. `callAsync` after `attached()` still CreateWebView2 while Cubase owns the IPlugView message stack and hangs Cubase14.exe. IPlugView only parents the park HWND. Zip index stays lazy.
...
- Two VST3 instances in one host process must not share a WebView2 user-data folder. One profile for all instances serializes Chromium and stutters both UIs. That folder is also `localStorage`: never store machine-wide Settings there. Theme/motion/fps/OS/polisher/Unit meters live in `UiSettings` (`%AppData%/NEUROKLAST/NeuroKore/ui.settings`) and ride the host snapshot. Mix, knobs, and the formula stay host state per insert. `setStateInformation` must not write OS/polisher back to AppData — re-apply from UiSettings after `replaceState`. `emitEventIfBrowserIsVisible` is a no-op when `Component::isVisible` is false — do not hide the parked WebView if shared prefs must reach it. Another process writes the same file: reload on change, do not keep a stale singleton.
