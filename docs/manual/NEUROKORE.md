...
- Pan the board on empty pane, middle mouse, or space+drag. Wheel zooms under the cursor. Hover a block to blur the rest of the board: that block, its neighbours and their cables stay sharp. Move the pointer off the board and every block is sharp again. Drag a chip — it follows the pointer; cables meet it. The board stores the new place when you drop, then etches the PCB. The camera is not a React re-render.
