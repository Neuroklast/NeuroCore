...
**Stand:** 2026-08-26 (shared Settings live in AppData, not WebView2 localStorage)  
**Version:** 0.6.1-beta  
**Branch:** `master`
...
| WP2 WebView-Shell | **grün** — `tests/WebShellTest.h`. Web only. Processor owns the browser (`WebViewHolder`). Three lifetimes: processor / IPlugView frame / Chromium. Holder ctor, `createView`, and `attached()` do **not** spawn Chromium — `callAsync` + `shouldRealizeChromium` (scan already `removed()`). Editor close hides it (removeChild, no delete). Windows VST3 park HWND is a sibling of IPlugView, never a child — `DestroyWindow` runs before `~Editor`. Standalone park parent is the top-level editor peer (the app window), not the hidden owner HWND — otherwise the WebView stays parked off-screen. Reopen is 0 ms — zip index and JS heap stay. |
...
Gate 2026-08-22: **0.6.1-beta**. Headless Circuit (boardStore) + DIGICIDE. Artefakte: `build/NeuroKore_artefacts/Release/Standalone/NEUROKORE-0.6.1-beta.exe`, `build/NeuroKore_artefacts/Release/VST3/NEUROKORE-0.6.1-beta.vst3` (Mac: plus `.component`).
