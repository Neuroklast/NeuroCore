...
| **Denorm / idle** | `DSPUtils::ScopedDenormalsAreZero` = JUCE FTZ/DAZ around `processBlock`; idle/bypass `SmoothedValue::skip`; ADAA SIMD lanes via `get`/`set` | No per-sample `flushDenorm`; no O(n) smoother ticks when dry | SIMD ADAA (still serial) |
...
| Rest | Factory: `resources/factory_presets.json` (306). Cyberpunk amps are Mesa tubes + one fine `round` (≥512) + 13-bit polish. `Chaotic Stab Generator` is untouched; sibling `Cyberpunk Stab` adds analog sub-octaver and a 10 % bright hall. Insert Level defaults sit near unity; guitar/distortion cab LPF ≥ ~6.5 kHz unless the filter is the effect. `out` is last — post-mix limit/glue lives on each bus, not after `out`. Vocals 45 bleiben. Nie `generate_factory_presets.mjs` gegen den Shipping-Katalog. CMake/Vite binden erst beim Build. `split` expander ignores `#` / `//` comments (`# d Split:` is not a split block). |
...
Gate 2026-08-21: **0.5.1-alpha**. Factory loudness/air + cyberpunk amps. Unit: no pixel XY HUD; Time/Samples are a real waveform. Sanitation-Kette nach DSL. DSP-Runtime: Tape → optional asmjit (Win x64 Arithmetik) → `processBlock`-Dispatch → 64-align Ringe → Plugin-LTO. macOS: VST3+AU, Tape, WKWebView. Artefakte: `build/NeuroKore_artefacts/Release/Standalone/NEUROKORE-0.5.1-alpha.exe`, `build/NeuroKore_artefacts/Release/VST3/NEUROKORE-0.5.1-alpha.vst3` (Mac: plus `.component`).
