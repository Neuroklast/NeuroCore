...
| WP5 Assemble | **grün** — React Flow: easy-connect, proximity, pulse-edges, context menu, zoom-tiers, validation, L→R. ChipSpec catalog (`chipSpec` / `typeCode` / `minBodyPx`); expand no longer grows the chip. Inspect: enum `<select>` from `chipSpec.enums`, numeric clamp to `ranges`, blurb + bound knobs (unit, 2 dp). Tubes: `segmentAngle` ∈ {0,45,90}; cable kinds audio red+flow / param yellow / LFO blue; arrange `ARR_CHIP_GAP` + foreign-tube clearance. |
