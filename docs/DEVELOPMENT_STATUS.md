...
| WP2 WebView-Shell | **grün** — `tests/WebShellTest.h`. Web only. Processor owns the browser (`WebViewHolder`); editor close hides it (removeChild, no delete). Windows VST3 park HWND is a sibling of IPlugView, never a child — `DestroyWindow` runs before `~Editor`. Standalone park parent is the top-level editor peer (the app window), not the hidden owner HWND — otherwise the WebView stays parked off-screen. Reopen is 0 ms — zip index and JS heap stay. |
