...
std::string PresetManager::encrypt(const std::string &text) const {
  ...
  juce::BlowFish bf(key.toRawUTF8(), (int)key.getNumBytesAsUTF8());
  bf.encrypt(data);
  return juce::Base64::toBase64(data.getData(), data.getSize()).toStdString();
}
...
std::string PresetManager::decrypt(const std::string &text) const {
  ...
  if (!juce::Base64::convertFromBase64(out, text))
    return {};
  juce::MemoryBlock data(out.getData(), out.getDataSize());
  juce::BlowFish bf(key.toRawUTF8(), (int)key.getNumBytesAsUTF8());
  bf.decrypt(data);
  return std::string(static_cast<const char *>(data.getData()), data.getSize());
}
...
bool PresetManager::loadPreset(const juce::File &file) {
  ...
  if (j.contains("variables")) {
    ...
    for (size_t i = 0;
         i < std::min(vars.size(), processor.getVariableNames().size()); ++i)
      ...
      if (params.contains(paramID.toStdString())) {
        ...
        if (parameter != nullptr) {
          parameter->setValueNotifyingHost(
              params[paramID.toStdString()].get<float>());
        }
      }
    }
  }
...
PresetManager::getAvailablePresets(const juce::File &dir) const {
