...
Artefacts land in `build/NeuroKore_artefacts/Release/` (`Standalone/NEUROKORE-0.4.10-alpha.exe`, `VST3/NEUROKORE-0.4.10-alpha.vst3`, and on macOS `AU/NEUROKORE-0.4.10-alpha.component`). The web editor is embedded in those binaries. A macOS build also copies the AU to `~/Library/Audio/Plug-Ins/Components/`.
...
That stages `NEUROKORE-0.4.10-alpha.vst3` and `NEUROKORE-0.4.10-alpha.exe`, writes `build/package/NEUROKORE-0.4.10-alpha-win64.zip`, fills the portable kit `NEUROKORE-0.4.10-alpha/` in the repo root (VST3, Standalone, Docs, EULA), and compiles `installer/NeuroKore.iss` if [Inno Setup 6](https://jrsoftware.org/isinfo.php) is installed (`ISCC.exe`). The installer copies the VST3 to `C:\Program Files\Common Files\VST3\`. Testers only need the VST3 and the exe — no `web/` folder.
