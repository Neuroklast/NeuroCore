...
Artefacts land in `build/NeuroKore_artefacts/Release/` (`Standalone/`, `VST3/NEUROKORE.vst3`, and on macOS `AU/NEUROKORE.component`). Resources are copied next to each binary. A macOS build also copies the AU to `~/Library/Audio/Plug-Ins/Components/`.
...
That stages `NEUROKORE.vst3` and the Standalone, writes `build/package/NEUROKORE-0.9.0-win64.zip`, fills the portable kit `NEUROKORE-0.9.0/` in the repo root (VST3, Standalone, Docs, EULA), and compiles `installer/NeuroCore.iss` if [Inno Setup 6](https://jrsoftware.org/isinfo.php) is installed (`ISCC.exe`). The installer copies the VST3 to `C:\Program Files\Common Files\VST3\`.
...
2. Open **Presets** and pick a factory preset (amp, delay, vocal chain, …). Use **<** / **>** next to the name chip to step through the library.
...
5. **Functions** is a catalog with folders on the left: **Core** (math), **Drive** (tube/diode/clip), **Crush** (fold/bitcrush), **Blocks** (ott, widen, vocoder, …).
