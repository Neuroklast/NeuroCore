...
4. **Graph** is the board: parts snap to a faint rose-cross grid and look like chips you assemble. Right-click to add Bus, Mid-Side Split, L/R Split, Crossover. Jacks light when a cable can drop.
5. **Script** is the hack: the same construct as text. Live view shows `a[3.20]`. **Edit** opens the editor; **Save** applies it.
6. **Functions** is a catalog with folders on the left: **Core** (math), **Drive** (tube/diode/clip), **Crush** (fold/bitcrush), **Blocks** (ott, widen, vocoder, …).
