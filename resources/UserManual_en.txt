...
Arrange and Compact
- Right-click empty board → Arrange (one row, left to right) or Compact (wrap to the window, still left to right — never a snake). The wrap cable runs between the rows, not around the whole board. Ctrl+A Arrange, Shift+A Compact. Loading a preset arranges, and wraps only if the chain is wider than the window. Dragging a chip stops auto-layout until you Arrange/Compact again.
